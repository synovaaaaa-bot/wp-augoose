...
function wp_augoose_handle_payment_result_redirect( $result, $order_id ) {
	...
	if ( isset( $result['result'] ) && 'failure' === $result['result'] ) {
		...
		// CRITICAL: ALWAYS replace order-received redirect with Doku payment URL
		...
		// CRITICAL: If redirect is order-received, we MUST get Doku URL and replace it
		// DOKU should NEVER redirect to order-received - always to payment page
			...
			try {
				...
				if ( empty( $doku_payment_url ) ) {
					...
					if ( isset( $gateway[ $payment_method ] ) ) {
						...
						// This is critical - DOKU gateway might not return URL in first call
						if ( empty( $doku_payment_url ) && method_exists( $doku_gateway, 'process_payment' ) ) {
							...
							// Suppress any output/errors during process_payment
							ob_start();
							try {
								...
								ob_end_clean();
								if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
									error_log( 'DOKU Payment: Error in process_payment: ' . $e->getMessage() );
								}
							}
						}
						
						// Method 4: Check if DOKU stored URL in order notes or other places
						if ( empty( $doku_payment_url ) ) {
							// Check order notes for Doku URL
							$order_notes = wc_get_order_notes( array( 'order_id' => $order_id ) );
							foreach ( $order_notes as $note ) {
								$note_content = $note->comment_content;
								if ( strpos( $note_content, 'checkout.doku.com' ) !== false ||
								     strpos( $note_content, 'checkout-link-v2' ) !== false ) {
									// Try to extract URL from note
									preg_match( '#https?://[^\s<>"]+#', $note_content, $matches );
									if ( ! empty( $matches[0] ) ) {
										$doku_payment_url = $matches[0];
										break;
									}
								}
							}
				...
				if ( ! empty( $doku_payment_url ) ) {
					...
					if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
						error_log( 'DOKU Payment: Found payment URL for order #' . $order_id . ': ' . $doku_payment_url );
					}
				} else {
					// CRITICAL: If still no Doku URL, we MUST get it by processing payment
					// Don't redirect to order-pay - get URL directly
					$gateway = WC()->payment_gateways->get_available_payment_gateways();
					if ( isset( $gateway[ $payment_method ] ) ) {
						$doku_gateway = $gateway[ $payment_method ];
						
						// Force process payment to get redirect URL
						if ( method_exists( $doku_gateway, 'process_payment' ) ) {
							...
							// Process payment - this should return Doku payment URL
							ob_start();
							try {
								$payment_result = $doku_gateway->process_payment( $order_id );
								ob_end_clean();
								...
								if ( isset( $payment_result['result'] ) && $payment_result['result'] === 'success' && ! empty( $payment_result['redirect'] ) ) {
									// Check if it's a Doku URL - if not, it might be order-received
									$payment_redirect = $payment_result['redirect'];
									$is_doku_redirect = (
										strpos( $payment_redirect, 'checkout.doku.com' ) !== false ||
										strpos( $payment_redirect, 'checkout-link-v2' ) !== false ||
										strpos( $payment_redirect, 'doku.com' ) !== false ||
										strpos( $payment_redirect, 'jokul' ) !== false
									);
									
									if ( $is_doku_redirect ) {
										// Use the redirect URL from process_payment
										$result['redirect'] = $payment_redirect;
										
										// Store it
										try {
											$order->update_meta_data( '_doku_redirect_url', $payment_redirect );
											$order->save_meta_data();
										} catch ( Exception $e ) {
											// Ignore
										}
										...
										if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
											error_log( 'DOKU Payment: Got payment URL from process_payment for order #' . $order_id . ': ' . $payment_redirect );
										}
									} else {
										// process_payment returned non-Doku URL (probably order-received)
										// Try to get URL from order meta or gateway properties
										if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
											error_log( 'DOKU Payment: process_payment returned non-Doku URL for order #' . $order_id . ': ' . $payment_redirect );
										}
										// Continue to search for Doku URL below
									}
								}
								
								// If we still don't have Doku URL, try to get from gateway properties
								if ( empty( $result['redirect'] ) || strpos( $result['redirect'], 'order-received' ) !== false ) {
									// Try to access gateway properties that might contain payment URL
									if ( isset( $doku_gateway->payment_url ) && ! empty( $doku_gateway->payment_url ) ) {
										$gateway_url = $doku_gateway->payment_url;
										if ( strpos( $gateway_url, 'checkout.doku.com' ) !== false ||
										     strpos( $gateway_url, 'checkout-link-v2' ) !== false ||
										     strpos( $gateway_url, 'doku.com' ) !== false ||
										     strpos( $gateway_url, 'jokul' ) !== false ) {
											$result['redirect'] = $gateway_url;
											try {
												$order->update_meta_data( '_doku_redirect_url', $gateway_url );
												$order->save_meta_data();
											} catch ( Exception $e ) {
												// Ignore
											}
									...
									// Last resort: redirect to order-pay (not order-received)
									if ( empty( $result['redirect'] ) || strpos( $result['redirect'], 'order-received' ) !== false ) {
										$result['redirect'] = $order->get_checkout_payment_url( true );
										if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
											error_log( 'DOKU Payment: Could not get Doku URL, redirecting to order-pay for order #' . $order_id );
										}
									}
								}
							} catch ( Exception $e ) {
								ob_end_clean();
								// Last resort: redirect to order-pay
								$result['redirect'] = $order->get_checkout_payment_url( true );
								if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
									error_log( 'DOKU Payment: Error in process_payment for order #' . $order_id . ': ' . $e->getMessage() );
								}
							}
						} else {
							// Gateway doesn't have process_payment - redirect to order-pay
							$result['redirect'] = $order->get_checkout_payment_url( true );
						}
					} else {
						// Gateway not found - redirect to order-pay
						$result['redirect'] = $order->get_checkout_payment_url( true );
					}
				}
			} catch ( Exception $e ) {
		...
		// CRITICAL: Final check - if redirect is still order-received, we MUST replace it
		// This is a safety net to ensure we NEVER redirect to order-received for DOKU
		if ( ! empty( $result['redirect'] ) && strpos( $result['redirect'], 'order-received' ) !== false ) {
			// Try one more time to get Doku URL from order meta
			$doku_url = $order->get_meta( '_doku_redirect_url' );
			if ( ! empty( $doku_url ) && (
				strpos( $doku_url, 'checkout.doku.com' ) !== false ||
				strpos( $doku_url, 'checkout-link-v2' ) !== false ||
				strpos( $doku_url, 'doku.com' ) !== false ||
				strpos( $doku_url, 'jokul' ) !== false
			) ) {
				$result['redirect'] = $doku_url;
			} else {
				// Last resort: redirect to order-pay (NOT order-received)
				$result['redirect'] = $order->get_checkout_payment_url( true );
				if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
					error_log( 'DOKU Payment: CRITICAL - Redirect was order-received, replaced with order-pay for order #' . $order_id );
				}
			}
		}
		
		// Ensure redirect URL is valid and is NOT order-received
		if ( ! empty( $result['redirect'] ) ) {
			$result['redirect'] = esc_url_raw( $result['redirect'] );
			
			// Final safety check - if somehow still order-received, force to order-pay
			if ( strpos( $result['redirect'], 'order-received' ) !== false ) {
				$result['redirect'] = $order->get_checkout_payment_url( true );
				if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
					error_log( 'DOKU Payment: CRITICAL - Final check found order-received, forcing order-pay for order #' . $order_id );
				}
			}
		}
	}
...
/**
 * Intercept DOKU process_payment result to capture payment URL
 * This runs immediately after process_payment to capture the redirect URL
 */
add_filter( 'woocommerce_payment_successful_result', 'wp_augoose_intercept_doku_process_payment_result', 1, 2 );
function wp_augoose_intercept_doku_process_payment_result( $result, $order_id ) {
	...
	// If redirect URL is order-received, we need to get Doku URL immediately
	if ( ! empty( $result['redirect'] ) && strpos( $result['redirect'], 'order-received' ) !== false ) {
		// Try to get Doku URL from order meta first
		$doku_url = $order->get_meta( '_doku_redirect_url' );
		if ( ! empty( $doku_url ) && (
			strpos( $doku_url, 'checkout.doku.com' ) !== false ||
			strpos( $doku_url, 'checkout-link-v2' ) !== false ||
			...
			$result['redirect'] = $doku_url;
			if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
				error_log( 'DOKU Payment: Intercepted order-received redirect, replaced with Doku URL from meta for order #' . $order_id );
			}
		} else {
			...
			if ( isset( $gateway[ $payment_method ] ) ) {
				...
				// Check gateway properties for payment URL
				if ( isset( $doku_gateway->payment_url ) && ! empty( $doku_gateway->payment_url ) ) {
					$gateway_url = $doku_gateway->payment_url;
					if ( strpos( $gateway_url, 'checkout.doku.com' ) !== false ||
					     strpos( $gateway_url, 'checkout-link-v2' ) !== false ||
						...
						$order->save_meta_data();
						if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
							error_log( 'DOKU Payment: Intercepted order-received redirect, replaced with Doku URL from gateway for order #' . $order_id );
						}
					}
				}
			}
...
/**
 * Capture DOKU payment URL when order is processed
 * This runs after order is created but before payment processing
 * CRITICAL: This ensures URL is available immediately for redirect
 */
add_action( 'woocommerce_checkout_order_processed', 'wp_augoose_capture_doku_url_on_order_processed', 20, 3 );
function wp_augoose_capture_doku_url_on_order_processed( $order_id, $posted_data, $order ) {
	if ( ! $order || ! is_a( $order, 'WC_Order' ) ) {
		return;
	}
	...
	// Try to get payment URL from gateway immediately after order creation
	try {
		$gateway = WC()->payment_gateways->get_available_payment_gateways();
		if ( isset( $gateway[ $payment_method ] ) ) {
			$doku_gateway = $gateway[ $payment_method ];
			
			// Method 1: Try get_payment_url method
			if ( method_exists( $doku_gateway, 'get_payment_url' ) ) {
				$payment_url = $doku_gateway->get_payment_url( $order_id );
				if ( ! empty( $payment_url ) && (
					strpos( $payment_url, 'checkout.doku.com' ) !== false ||
			...
			// Method 2: Try process_payment to get URL
			if ( method_exists( $doku_gateway, 'process_payment' ) ) {
				// Set order as awaiting payment
				if ( WC()->session ) {
					WC()->session->set( 'order_awaiting_payment', $order_id );
				...
				try {
					...
					ob_end_clean();
					// Ignore error - will be handled in payment_successful_result filter
				}
			}
		}
	} catch ( Exception $e ) {
...
/**
 * Prevent cart from being cleared for DOKU payments before redirect
 * This ensures session doesn't expire before redirect to Doku
 */
add_action( 'woocommerce_checkout_order_processed', 'wp_augoose_preserve_cart_for_doku', 5, 3 );
function wp_augoose_preserve_cart_for_doku( $order_id, $posted_data, $order ) {
	if ( ! $order || ! is_a( $order, 'WC_Order' ) ) {
		return;
	}
	...
	// Don't clear cart immediately for DOKU - let it clear after redirect
	// This prevents "session expired" error
	if ( WC()->session ) {
		// Keep order_awaiting_payment in session for redirect
		WC()->session->set( 'order_awaiting_payment', $order_id );
		WC()->session->save_data();
	}
}
 ...
 * CRITICAL: This must run early to intercept order-pay page before WooCommerce renders it
 */
add_action( 'template_redirect', 'wp_augoose_handle_order_pay_doku_redirect', 1 );
function wp_augoose_handle_order_pay_doku_redirect() {
	...
	// For ALL DOKU orders that need payment, ALWAYS redirect to Doku payment page
	// This handles the case where invoice is in email but no direct redirect
	// Don't check status - just check if order needs payment
	if ( $order->needs_payment() ) {
		$doku_payment_url = null;
		
		// Method 1: Try to get payment URL from order meta first
		$meta_keys = array( 
			'_doku_redirect_url', 
			'_doku_payment_url', 
			'_jokul_payment_url', 
		...
		foreach ( $meta_keys as $meta_key ) {
			$meta_value = $order->get_meta( $meta_key );
			if ( ! empty( $meta_value ) && (
				strpos( $meta_value, 'checkout.doku.com' ) !== false ||
				strpos( $meta_value, 'checkout-link-v2' ) !== false ||
				strpos( $meta_value, 'doku.com' ) !== false ||
				strpos( $meta_value, 'jokul' ) !== false
			) ) {
		...
		// Method 2: Search ALL order meta for Doku URL
		if ( empty( $doku_payment_url ) ) {
			$all_meta = $order->get_meta_data();
			foreach ( $all_meta as $meta ) {
				$meta_value = $meta->value;
		...
		// Method 3: Try to get from gateway
		if ( empty( $doku_payment_url ) ) {
			$gateway = WC()->payment_gateways->get_available_payment_gateways();
			if ( isset( $gateway[ $payment_method ] ) ) {
				$doku_gateway = $gateway[ $payment_method ];
				...
				// Method 4: Process payment to get redirect URL
				if ( empty( $doku_payment_url ) && method_exists( $doku_gateway, 'process_payment' ) ) {
					// Set order as awaiting payment
					if ( WC()->session ) {
						WC()->session->set( 'order_awaiting_payment', $order_id );
					...
					try {
						...
						ob_end_clean();
						if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
							error_log( 'DOKU Payment: Error in process_payment on order-pay: ' . $e->getMessage() );
						}
					}
				}
			}
		...
		// If we found Doku payment URL, redirect immediately
		if ( ! empty( $doku_payment_url ) ) {
			// Store it for future use
			try {
				$order->update_meta_data( '_doku_redirect_url', $doku_payment_url );
			...
			wp_safe_redirect( $doku_payment_url );
			exit;
		}
...
// After the item has been inserted into the cart we also force the product
// object to use the converted IDR price (if available). This guarantees that
// `WC()->cart->get_cart_contents_total()` and all WooCommerce totals reflect
// the converted value immediately, which is important for AJAX fragments
// such as the mini‑cart.
add_filter( 'woocommerce_add_cart_item', 'wp_augoose_apply_idr_price_to_cart_item', 20, 1 );

function wp_augoose_apply_idr_price_to_cart_item( $cart_item ) {
    if ( isset( $cart_item['wp_augoose_converted_price_idr'] ) && $cart_item['wp_augoose_converted_price_idr'] > 0 ) {
        $product = isset( $cart_item['data'] ) ? $cart_item['data'] : null;
        if ( $product && is_a( $product, 'WC_Product' ) ) {
            try {
                $product->set_price( (float) $cart_item['wp_augoose_converted_price_idr'] );
                if ( isset( $product->data ) && is_array( $product->data ) ) {
                    $product->data['price'] = (string) $cart_item['wp_augoose_converted_price_idr'];
                }
                // also update cart_contents directly in case calculation has
                // already run once during this request
                if ( function_exists( 'WC' ) && WC()->cart ) {
                    foreach ( WC()->cart->get_cart() as $key => $item ) {
                        if ( $key === $cart_item['key'] ) {
                            WC()->cart->cart_contents[ $key ]['data'] = $product;
                            break;
                        }
                    }
                }
                ...
                if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
                    error_log( 'WP_Augoose: Error applying IDR price to cart item - ' . $e->getMessage() );
                }
            }
        }
    }
...
function wp_augoose_save_original_currency_to_cart_item( $cart_item_data, $product_id, $variation_id ) {
	...
	// Determine client currency – prefer WCML but fall back to cookie
	$client_currency = wp_augoose_safe_get_client_currency();
	
	// If WCML returned nothing, try the manual cookie
	...
	// **Important:** if the shop currency has already been forced to IDR
	// (e.g. Indonesian IP) we override whatever we read above. This avoids
	// cases where a stale cookie/previous session says SGD but the store is
	// already showing IDR prices.
	...
	// IMPORTANT: For USD or IDR, save currency but don't convert
	if ( $client_currency === 'USD' || $client_currency === 'IDR' ) {
		...
		return $cart_item_data; // USD and IDR stay as-is
	}
	
	// Skip if not SGD/MYR (only these two get converted to IDR)
	...
	// This is the price user saw on product page (e.g., 80 SGD, or 750000 IDR
	// if the store currency has already flipped). If the store is IDR we
	// reverse‑convert the displayed price so `original_price` always represents
	// the amount in the customer's currency.
	$displayed_price = (float) $product->get_price( 'edit' );
	if ( $displayed_price <= 0 ) {
		return $cart_item_data;
	}
	
	// figure out original price in customer currency
	$original_price = $displayed_price;
	$store_currency = function_exists( 'get_woocommerce_currency' ) ? get_woocommerce_currency() : '';
	if ( $store_currency === 'IDR' && $client_currency && in_array( $client_currency, array( 'SGD', 'MYR' ), true ) ) {
		$reverse_rate = wp_augoose_safe_get_exchange_rate_to_idr( $client_currency );
		if ( $reverse_rate && $reverse_rate > 0 ) {
			$original_price = round( $displayed_price / $reverse_rate, 2 );
		}
	}
	
	// Get WCML exchange rate to convert original price to IDR
	$rate = wp_augoose_safe_get_exchange_rate_to_idr( $client_currency );
	$price_idr = null;
	if ( $rate && $rate > 0 ) {
		$price_idr = round( $original_price * $rate, 2 );
		if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
			error_log( "WP_Augoose: Add to cart - Product #{$product_id}, {$client_currency} {$original_price} → IDR {$price_idr} (rate: {$rate})" );
		}
	}
	
	// Save to cart item data (store both original and displayed for debugging)
	if ( $price_idr && $price_idr > 0 ) {
		$cart_item_data['wp_augoose_original_currency'] = $client_currency;
		$cart_item_data['wp_augoose_original_price'] = $original_price;
		$cart_item_data['wp_augoose_displayed_price'] = $displayed_price;
		$cart_item_data['wp_augoose_converted_price_idr'] = $price_idr;
		$cart_item_data['wp_augoose_converted_to_idr'] = true;
		
		if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
			error_log( "WP_Augoose: Saved conversion data - currency: {$client_currency}, original: {$original_price}, displayed: {$displayed_price}, converted: {$price_idr}" );
		}
	}
 ...
 * CRITICAL: This filter must work for ALL products (jacket, pants, etc.)
 */
...
function wp_augoose_ensure_variation_active_if_in_stock( $is_active, $variation ) {
	...
	// Method 1: WooCommerce methods
	$stock_quantity = $variation->get_stock_quantity();
	$stock_status = $variation->get_stock_status();
	$manage_stock = $variation->get_manage_stock();
	
	// Method 2: Direct meta access (more reliable)
	...
	// Method 3: Check parent product stock settings if variation doesn't manage stock
	$parent_id = $variation->get_parent_id();
	$parent_manage_stock = false;
	if ( $parent_id ) {
		$parent_manage_stock_meta = get_post_meta( $parent_id, '_manage_stock', true );
		if ( $parent_manage_stock_meta !== '' ) {
			$parent_manage_stock = wc_string_to_bool( $parent_manage_stock_meta );
		}
	}
	
	// Use meta values if get methods return null/empty/false
	if ( ( $stock_quantity === null || $stock_quantity === false ) && $stock_quantity_meta !== '' && $stock_quantity_meta !== null && $stock_quantity_meta !== false ) {
		$stock_quantity = (int) $stock_quantity_meta;
	}
	if ( empty( $stock_status ) && ! empty( $stock_status_meta ) ) {
		$stock_status = $stock_status_meta;
	}
	if ( ( $manage_stock === null || $manage_stock === false ) && $manage_stock_meta !== '' ) {
		$manage_stock = wc_string_to_bool( $manage_stock_meta );
	}
	
	// If parent manages stock but variation doesn't, check parent stock
	if ( ! $manage_stock && $parent_manage_stock ) {
		$parent_stock_quantity = get_post_meta( $parent_id, '_stock', true );
		if ( $parent_stock_quantity !== '' && $parent_stock_quantity !== null && (int) $parent_stock_quantity > 0 ) {
			$stock_quantity = (int) $parent_stock_quantity;
			$manage_stock = true;
		}
	}
	
	// CRITICAL: If stock quantity exists and > 0, ALWAYS make it active (regardless of manage_stock setting)
	if ( $stock_quantity !== null && $stock_quantity !== false && (int) $stock_quantity > 0 ) {
		return true;
	}
	
	// If stock is managed and quantity > 0, make it active
	if ( $manage_stock && $stock_quantity !== null && $stock_quantity !== false && (int) $stock_quantity > 0 ) {
		return true;
	}
	...
	// If variation is published and has a price, make it active (even without stock management)
	if ( $variation->get_status() === 'publish' && $variation->get_price() ) {
		// Only if stock quantity is not explicitly 0
		if ( $stock_quantity === null || $stock_quantity === false || (int) $stock_quantity > 0 ) {
			return true;
		}
	}
...
function wp_augoose_ensure_variation_purchasable_if_in_stock( $is_purchasable, $variation ) {
	...
	// CRITICAL: If stock quantity exists and > 0, ALWAYS make it purchasable (regardless of manage_stock setting)
	if ( $stock_quantity !== null && $stock_quantity !== false && (int) $stock_quantity > 0 ) {
		return true;
	}
	...
	// If variation is published and has a price, make it purchasable (even without stock management)
	if ( $variation->get_status() === 'publish' && $variation->get_price() ) {
		// Only if stock quantity is not explicitly 0
		if ( $stock_quantity === null || $stock_quantity === false || (int) $stock_quantity > 0 ) {
			return true;
...
function wp_augoose_ensure_variation_available_if_in_stock( $variation_data, $product, $variation ) {
	...
	// CRITICAL: If stock quantity exists and > 0, ALWAYS consider it has stock (regardless of manage_stock setting)
	if ( $stock_quantity !== null && $stock_quantity !== false && (int) $stock_quantity > 0 ) {
		$has_stock = true;
	}
